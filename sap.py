...
def flight():
	...
	#this is kind of broken, since in this test we don't care wich flight, but how much of each duplicate flight. Fix that, but make it a feature.
	#print flights,"\n", times
...
#the time needs to be polished, since there is a lot of possible times with all those minutes and such, so I need to make it so that the user can pick wich range of times to choose from, in this test, we are assuming the range is from 6:00 pm to 7:00 pm, remember the time of day is very important
	print time()
	timeg = time()
